document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchQuestions() {
        const url = 'https://raw.githubusercontent.com/garyortega11-alt/Bible/main/assets/questions.json';
        try {
            ...
            alert('Error crítico: No se pudieron cargar las preguntas del juego.');
            throw error;
        }
    }
    ...
    function displayQuestion() {
        ...
        const options = shuffleArray(question.options.map(opt => ({ text: opt, isCorrect: opt === question.correctAnswer })));
    ...
    function showAnswerTooltip(isCorrect, question) {
        ...
        document.getElementById('tooltip-correct-answer-text').textContent = `Respuesta: ${question.correctAnswer}`;
    ...
    function checkLevelUpAnswer(selectedButton, question) {
        ...
        const correctButton = options.find(btn => btn.dataset.isCorrect === 'true');
    ...
    function wildcard5050() {
        ...
        const incorrectOptions = options.filter(btn => btn.dataset.isCorrect !== 'true');
